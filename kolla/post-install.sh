...
tee > ~/open.rc <<EOF
#!/usr/bin/env bash
export OS_AUTH_URL=http://cloud.rvbdemo.com:5000/v3
...
echo "Login using http://cloud.rvbdemo.com with default as domain,  admin as username, and $(cat /etc/kolla/passwords.yml | grep "keystone_admin_password" | awk '{print $2}') as password"
